/*
  Run this script with `npm run pub`

  This script will upload the compiled site to Filecoin, then publish the new
  hash to the BCH blockchain. The page at paperwallet.fullstack.cash will
  automatically redirect users to the new site.
*/

const FILE_PATH = './zensurresistent'

require('dotenv').config()

// Public npm libraries
const { Web3Storage, getFilesFromPath } = require('web3.storage')
const BchWallet = require('../../psf/minimal-slp-wallet/index')
const BchMessageLib = require('../../psf/bch-message-lib/index')
const fs = require('fs')
...
async function publish() {
  try {
    ...
    const zensurresistWif = process.env.PAPERWALLET_WIF
    if (!zensurresistWif) {
      throw new Error(
        'WIF for paperwallet.fullstack.cash not detect. Add it to the PAPERWALLET_WIF environment variable.'
      )
    }
    ...
    const wallet = new BchWallet(zensurresistWif, {
